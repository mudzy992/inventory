    ...
    exec("sh ./deploy-scripts/deploy.sh", (error, stdout, stderr) => {
      if (error) {
        ...
        res
          .status(500)
          .send(
            `Greška prilikom izvršavanja deploy.sh! Greška: ${error} ili ${stderr}`
          );
        return;
      }
      console.log(
        `Skripta deploy.sh za backend je uspješno izvršena: ${stdout}`
      );
      res
        .status(200)
        .send(`Webhook primljen i deployiran za BackEnd. Stanje: ${stdout}`);
    });
  }
