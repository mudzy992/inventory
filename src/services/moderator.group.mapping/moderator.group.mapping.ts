...
export class ModeratorGroupMappingService extends TypeOrmCrudService<ModeratorGroupMapping> {
  ...
  async getAllGroupModerators(): Promise<ModeratorGroupMappingDTO[] | ApiResponse> {
    ...
    const response: ModeratorGroupMappingDTO[] = await Promise.all(moderators.map(async (item) => {
      const subGroups = await this.ticketGroup.find({where: {parentGroupId: item.groupId}})
      return {
        ...
        group: {
          ...
          ticketGroups: subGroups ? subGroups.map((sub) => ({
            groupId: sub.groupId,
